  ...
  useEffect(() => {
    ...
    if (isRemember) {
      if (isRemember?.isAdmin) {
        ...
        navigation.push(`/dashboardhotbeach/home?hotel=${hotel}`);
      }
    } else {
      setLoggedIn(false);
    ...
    if (search !== "") {
      const filteredUsers = currentUsers?.filter(
        (user) =>
          ...
          (user?.hotel === hotel || user?.hotel === "TODOS")
      );
...
function CreateNewUser({
  ...
  const searchParams = useSearchParams();
  const currentHotel = searchParams.get("hotel") || "HOT_BEACH_RESORT";
  ...
  const form = useForm<z.infer<typeof schemaUser>>({
    ...
    defaultValues: {
      ...
      password: "",
      isAdmin: false,
      hotel: currentHotel as "MABU_THERMAS" | "HOT_BEACH_RESORT" | "HOT_BEACH_SUITE" | "JAPARATINGA" | "TODOS"
    }
  });
