...
export function CreateReservas({
    ...
    const hotelDaReserva = (reserva?.cotacao as { hotel?: string })?.hotel ?? (cotacaoAlter as { hotel?: string })?.hotel;
    const isJaparatinga = hotel === "JAPARATINGA" || hotelDaReserva === "JAPARATINGA";
    ...
    useEffect(() => {
        ...
        } else if (isJaparatinga && cotacao) {
            form.setValue('pensao', 'Pensão Completa');
        }
    }, [cotacaoAlter, cotacao, form, isJaparatinga, reserva?.adicional, reserva?.cotacao?.emailUsuario, reserva?.cotacao?.nomeUsuario, reserva?.opcao, reserva?.valorAdicional, reserva?.quantidadeDeAcompanhantes, reserva?.reservaAcompanhante, reserva?.rgCPF, reserva?.status, reserva?.telefone, reserva?.checkoutUrl]);
        ...
        try {
            ...
            if (response.data.status) {
                if (cotacaoAlter && fecthUsers) {
                    ...
                    navigation.push(`/dashboardhotbeach/reservas?hotel=${hotel}`);
                }
            }
        } catch (error: any) {
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            {isJaparatinga ? (
                                                <div className="flex h-10 w-full items-center rounded-md border border-zinc-300 bg-gray-50 text-zinc-800 px-3 py-1 text-lg">
                                                    ALL INCLUSIVE {cotacao ? cotacao?.precoPensaoCompleta?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : Number(cotacaoAlter?.precoPensaoCompleta)?.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                                                </div>
                                            ) : (
                                                <select
