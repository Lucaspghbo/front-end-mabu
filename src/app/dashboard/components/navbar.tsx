...
export function Navbar({
  ...
  return (
    <>
      <aside
        className={`md:relative absolute top-0 md:w-[434px] ${showMenu
...
function PagamentoModalContent({
  ...
  const [hotel, setHotel] = useState("MABU_THERMAS");
  ...
  useEffect(() => {
    ...
    // Obtém o hotel do localStorage
    const hotelFromStorage = localStorage.getItem("hotel") || "MABU_THERMAS";
    setHotel(hotelFromStorage);

    api
      .get("/dados-pagamento")
      .then((res) => {
        // Filtra só o método do hotel atual
        const pagamentoDoHotel = res.data.find(
          (item: any) => item.hotel === hotelFromStorage
        );
