...
export default function HomeDash() {
  ...
  const fecthUsers = async () => {
    try {
      const response = await apiAdmin.get("/users?hotel=MABU_THERMAS");
  ...
  const fecthDataDashboard = async () => {
    try {
      const response = await api.get(
        `/userDashboard?dataInicial=${dataInicial}&dataFinal=${dataFinal}&status=${statusSelected}&hotel=MABU_THERMAS`
      );
      if (response.data) {
  ...
  const fecthDataDashboardAdminUsers = async (userId: string) => {
    setIsLoading(true);
    try {
      const response = await api.get(
        `/dashboard/admin/getUser/${userId}?dataInicial=${dataInicial}&dataFinal=${dataFinal}&status=${statusSelected}&hotel=MABU_THERMAS`
      );
      if (response.data) {
        setUserDashboard(response.data);
      }
